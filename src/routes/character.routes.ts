...
router.get("/language/:idioma", async (req, res) => {
    try {
        ...
        const characters = await CharacterService.getCharactersByLanguage(idioma);
        res.json(characters);
    } catch (error) {
...
router.get("/nationality/:nacionalidad", async (req, res) => {
    try {
        ...
        const characters = await CharacterService.getCharactersByNationality(nacionalidad);
        res.json(characters);
    } catch (error) {
...
router.post("/search", async (req, res) => {
    try {
        ...
        const characters = await CharacterService.searchCharacters(criteria);
        res.json(characters);
    } catch (error) {
...
router.get("/recommended/:userId", authenticateToken, async (req, res) => {
    try {
        ...
        const characters = await getRecommendedCharacters(
            idioma_objetivo as string,
...
router.post("/:characterId/chat", authenticateToken, async (req, res) => {
    try {
        const { characterId } = req.params;
        const userId = req.user._id;
        
        const result = await createChatWithCharacter(userId, characterId);
...
router.get("/random/language/:idioma", async (req, res) => {
    try {
        const { idioma } = req.params;
        const character = await CharacterService.getRandomCharacterByLanguage(idioma);
...
router.post("/:characterId/validate", async (req, res) => {
    try {
        ...
        const validation = CharacterService.validateCharacterConsistency(character, message);
...
router.get("/stats/overview", async (req, res) => {
    try {
        ...
        const stats = {
            ...
            por_idioma: {} as Record<string, number>,
            por_nacionalidad: {} as Record<string, number>,
            por_genero: { M: 0, F: 0 },
            por_nivel_enseñanza: {} as Record<string, number>
        };
