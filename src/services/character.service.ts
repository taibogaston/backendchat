...
import { charactersSeed } from "../data/characters.seed";

export class CharacterService {
    /**
     * Inicializa la base de datos con personajes predefinidos
     */
    static async seedCharacters(): Promise<void> {
        try {
            ...
            if (existingCharacters === 0) {
                ...
                console.log(`✅ ${charactersSeed.length} personajes sembrados exitosamente`);
            } else {
    ...
    static async getAllCharacters(): Promise<ICharacter[]> {
        return await Character.find({ activo: true }).sort({ nombre: 1 });
    }

    /**
     * Obtiene un personaje por ID
     */
    static async getCharacterById(id: string): Promise<ICharacter | null> {
     ...
     */
    static async getCharacterByName(nombre: string): Promise<ICharacter | null> {
        return await Character.findOne({ nombre, activo: true });
    }

    /**
     * Obtiene personajes por idioma objetivo
     */
    static async getCharactersByLanguage(idioma: string): Promise<ICharacter[]> {
     ...
     */
    static async getCharactersByNationality(nacionalidad: string): Promise<ICharacter[]> {
        return await Character.find({ 
            nacionalidad, 
            activo: true 
        }).sort({ nombre: 1 });
     ...
     */
    static generateCharacterPrompt(character: ICharacter): string {
        const {
            nombre,
            nacionalidad,
            genero,
...
`;
    ...
    static validateCharacterConsistency(
        ...
        const hasForbiddenLanguage = allowedLanguages.some(lang => 
        ...
        const hasTypicalExpression = typicalExpressions.some(expr => 
            message.toLowerCase().includes(expr.toLowerCase())
        );
     ...
     */
    static async getRandomCharacterByLanguage(idioma: string): Promise<ICharacter | null> {
    ...
    static async searchCharacters(criteria: {
        ...
        if (criteria.edad_min || criteria.edad_max) {
            ...
            if (criteria.edad_min) query["personalidad.edad"].$gte = criteria.edad_min;
            if (criteria.edad_max) query["personalidad.edad"].$lte = criteria.edad_max;
        }
        
        return await Character.find(query).sort({ nombre: 1 });
    }
}
